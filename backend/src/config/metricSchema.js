const { DEFAULT_VARIANTS } = require('./defaultConfigs');

const DOMAIN_METRIC_KEYS = Object.fromEntries(
    Object.entries(DEFAULT_VARIANTS).map(([domain, variant]) => {
        const keys = new Set();
        (variant.widgets || []).forEach((widget) => {
